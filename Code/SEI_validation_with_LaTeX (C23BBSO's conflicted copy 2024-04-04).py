...
#---------------------------------------------------------------------------------
# Figure path
figure_path = "/home/johborgq/Dropbox/Work/fancy_research_images/SEI_parameter_symmetry_2D"
# Save things in LaTeX as well
# Delta vs muE
for index in range(len(clr_vec)):
    #for sub_index in range(len(var_eps_vec)-15):
    for sub_index in range(0, len(var_eps_vec)-5, 5):
        if sub_index == 0:
            plot_LaTeX_2D(p_hat[index][sub_index:sub_index+5,5],p_hat[index][sub_index:sub_index+5,2],figure_path + "/Input/muE_vs_delta.tex","color=clr_" + str(index+1) + ",line width=1.5pt,-latex,->",[])
        else:
            plot_LaTeX_2D(p_hat[index][sub_index-1:sub_index+4,5],p_hat[index][sub_index-1:sub_index+4,2],figure_path + "/Input/muE_vs_delta.tex","color=clr_" + str(index+1) + ",line width=1.5pt,-latex,->",[])            
    #dashed,-latex,<-
# beta vs kI
for index in range(len(clr_vec)):
    plot_LaTeX_2D(p_hat[index][:,8],p_hat[index][:,1],figure_path + "/Input/beta_vs_kI.tex","color=clr_" + str(index+1) + ",line width=1.5pt",[])
# Delta vs muE
for index in range(len(clr_vec)):
    plot_LaTeX_2D(p_hat[index][:,5],p_hat[index][:,2],figure_path + "/Input/delta_vs_epsilon.tex","color=clr_" + str(index+1) + ",line width=1.5pt",[])
...
#---------------------------------------------------------------------------------
# Subplot A: uE vs delta
#---------------------------------------------------------------------------------    
for index in range(len(clr_vec)):
    # Plot the parameters against eachother yeah?
    ax_SEI_2D[0].plot(p_hat[index][:,5],p_hat[index][:,2],color=clr_vec[index],linewidth=3.0, linestyle = "dashdot")        
# Set the y-limit
#ax_SEI[0].set_ylim([0, 2.5])
# Set a grid and define a legend
ax_SEI_2D[0].grid()
...
for index in range(len(clr_vec)):
        ...
        # Plot the parameters against eachother yeah?
    ax_SEI_2D[1].plot(p_hat[index][:,8],p_hat[index][:,1],color=clr_vec[index],linewidth=3.0, linestyle = "dashdot")        
# Set the y-limit
